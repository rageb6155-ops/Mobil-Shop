...
namespace MobileShopSystem.Controllers
{
    public class SettingsController : Controller
    {
        ...
        // الحصول على جميع المستخدمين للرسائل الجماعية (محدث)
        [HttpGet]
            ...
            try
            {
                var users = await _context.Users
                    ...
                    .Select(u => new
                    {
                        ...
                        u.IsBlocked,
                        u.IsApproved,
                        u.IsSuspended, // ⭐ إضافة حالة التعليق
                        u.SuspensionMessage, // ⭐ إضافة رسالة التعليق
                        ...
                        Status = u.IsSuspended ? "معلق" : (u.IsBlocked ? "محظور" : (u.IsOnline ? "متصل" : "غير متصل")), // ⭐ تحديث الحالة
                        StatusText = u.IsSuspended ? "معلق" : (u.IsBlocked ? "محظور" : (u.IsOnline ? "متصل حالياً" : "غير متصل")) // ⭐ تحديث نص الحالة
                    })
        ...
        // إرسال رسالة واتساب لمستخدم واحد (محدث)
        [HttpPost]
        public async Task<IActionResult> SendWhatsAppMessage([FromBody] SendUserWhatsAppViewModel model)
        {
            try
            {
                ...
                string messageType = "";
                string userStatus = user.IsSuspended ? "معلق" : (user.IsBlocked ? "محظور" : (user.IsOnline ? "متصل حالياً" : "غير متصل")); // ⭐ تحديث الحالة
                ...
                switch (model.MessageType?.ToLower())
                {
                    ...
                    // ⭐ قالب جديد: تعليق الحساب
                    case "suspend":
                        var suspendMessage = $" *إشعار تعليق الحساب*\n\n";
                        suspendMessage += $"عزيزي {user.Username}،\n";
                        suspendMessage += $"تم تعليق حسابك في النظام مؤقتاً.\n\n";
                        suspendMessage += $"━━━━━━━━━━━━━━━━\n";
                        suspendMessage += $" *سبب التعليق:*\n";
                        suspendMessage += $"{model.CustomMessage}\n\n";
                        suspendMessage += $"━━━━━━━━━━━━━━━━\n";
                        suspendMessage += $" *بيانات حسابك:*\n";
                        suspendMessage += $" *اسم المستخدم:* {user.Username}\n";
                        suspendMessage += $" *رقم الهاتف:* {user.Phone}\n";
                        suspendMessage += $" *الحالة الحالية:* معلق\n";
                        suspendMessage += $" *الصلاحية:* {(user.IsAdmin ? "أدمن" : "مستخدم")}\n";
                        suspendMessage += $"━━━━━━━━━━━━━━━━\n";
                        suspendMessage += $" *للتواصل مع الإدارة:* 01064211484\n";
                        suspendMessage += $"━━━━━━━━━━━━━━━━";
                        response = await _whatsAppService.SendLongMessage(user.Phone, suspendMessage);
                        messageType = "تعليق";
                        break;

                    // ⭐ قالب جديد: إلغاء التعليق
                    case "unsuspend":
                        var unsuspendMessage = $" *إشعار إلغاء التعليق*\n\n";
                        unsuspendMessage += $"عزيزي {user.Username}،\n";
                        unsuspendMessage += $"تم إلغاء تعليق حسابك في النظام.\n";
                        unsuspendMessage += $"يمكنك الآن الدخول إلى حسابك كالمعتاد.\n\n";
                        unsuspendMessage += $"━━━━━━━━━━━━━━━━\n";
                        unsuspendMessage += $" *بيانات حسابك:*\n";
                        unsuspendMessage += $" *اسم المستخدم:* {user.Username}\n";
                        unsuspendMessage += $" *رقم الهاتف:* {user.Phone}\n";
                        unsuspendMessage += $" *الحالة الحالية:* نشط\n";
                        unsuspendMessage += $" *الصلاحية:* {(user.IsAdmin ? "أدمن" : "مستخدم")}\n";
                        unsuspendMessage += $"━━━━━━━━━━━━━━━━\n";
                        unsuspendMessage += $" *رابط الدخول:*\n{siteUrl}\n\n";
                        unsuspendMessage += $" للاستفسار: 01064211484\n";
                        unsuspendMessage += $"━━━━━━━━━━━━━━━━";
                        response = await _whatsAppService.SendLongMessage(user.Phone, unsuspendMessage);
                        messageType = "إلغاء تعليق";
                        break;
        ...
        // إرسال رسالة جماعية لمستخدمين محددين (محدث)
        [HttpPost]
        ...
        // ========== دوال المستخدمين الجديدة (تعليق/فك تعليق) ==========

        // ⭐ تعليق مستخدم (Suspend)
        [HttpPost]
        public async Task<IActionResult> Suspend(int id, string suspensionMessage)
        {
            try
            {
                ...
                {
                    TempData["ErrorMessage"] = "❌ ليس لديك صلاحية للقيام بهذا الإجراء";
                    return RedirectToAction("Index");
                }

                // لا يمكن تعليق حساب الأدمن نفسه
                if (user.Id == currentUser.Id)
                {
                    TempData["ErrorMessage"] = "❌ لا يمكنك تعليق حسابك الخاص";
                    return RedirectToAction("Index");
                }

                user.IsSuspended = true;
                user.SuspensionMessage = string.IsNullOrEmpty(suspensionMessage)
                    ? "حسابك معلق مؤقتاً. يرجى التواصل مع الإدارة."
                    : suspensionMessage;

                // مهم: إذا كان المستخدم متصلاً، نجبره على الخروج (نخليه offline)
                user.IsOnline = false;

                await _context.SaveChangesAsync();

                _logger.LogInformation("User {AdminUsername} suspended user {SuspendedUsername} with message: {Message}",
                    currentUsername, user.Username, suspensionMessage);

                TempData["SuccessMessage"] = $"✅ تم تعليق حساب {user.Username} بنجاح";
                return RedirectToAction("Index");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error suspending user {UserId}", id);
                TempData["ErrorMessage"] = "❌ حدث خطأ أثناء تعليق المستخدم";
                return RedirectToAction("Index");
            }
        }

        // ⭐ فك تعليق مستخدم (Unsuspend)
        [HttpPost]
        public async Task<IActionResult> Unsuspend(int id)
        {
            try
            {
                ...
                user.SuspensionMessage = null;

                await _context.SaveChangesAsync();

                _logger.LogInformation("User {AdminUsername} unsuspended user {UnsuspendedUsername}",
                    currentUsername, user.Username);

                TempData["SuccessMessage"] = $"✅ تم فك تعليق حساب {user.Username} بنجاح";
                return RedirectToAction("Index");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error unsuspending user {UserId}", id);
                TempData["ErrorMessage"] = "❌ حدث خطأ أثناء فك تعليق المستخدم";
                return RedirectToAction("Index");
            }
        }

        // ========== دوال المستخدمين الموجودة (بدون تغيير) ==========
        ...
        public async Task<IActionResult> Edit(User model)
        {
            ...
            // ⭐ إضافة إمكانية تعديل حالة التعليق ورسالة التعليق من صفحة التعديل
            user.IsSuspended = model.IsSuspended;
            user.SuspensionMessage = model.SuspensionMessage;
        ...
        // ===== دالة إضافية لتصحيح حالة المستخدمين (اختياري) =====
        [HttpGet]
        public async Task<IActionResult> FixUserStatus()
        {
            try
            {
                var onlineUsers = await _context.Users.Where(u => u.IsOnline).ToListAsync();
                return Json(new { success = true, message = $"تم العثور على {onlineUsers.Count} مستخدم متصل" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in FixUserStatus");
                return Json(new { success = false, message = ex.Message });
            }
        }
    }
